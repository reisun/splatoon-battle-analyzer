...
- 画面上部中央: 
    試合タイマー（残り時間）
- タイマーの左側: 
    自チームの色のイカランプ4つ（グレーアウト＝デス中）
- タイマーの右側: 
    相手チーム色のイカランプ4つ（グレーアウト＝デス中）
- 画面の右上端：
    スペシャルゲージ
- タイマーの下: 
    ゲームカウント。自チームの色と相手チームの色の２つ
    カウントの上に小さく「のこり」と表示されている
    先に0にすると勝ち=カウントが少ない方が勝っている
- ゲームカウント回りの小さな数字: 
    ルールごとに仕様が異なり複雑なため無視する。混同注意。
- 画面下部中央: 
    直近で倒したプレイヤーの名前「◯◯ をたおした！」と表示される
    以下間違えやすい表示。誤読しないようにすること。
    - 味方の名前。自チームの色が付いているため区別可能だが、誤読しないように注意。
    - スペシャル発動中の表示。「◯◯を打ちまくれ！」のように
    スペシャル発動中は同じエリアに文字が表示ことがある。倒した相手の名前ではないため誤読しないこと。
