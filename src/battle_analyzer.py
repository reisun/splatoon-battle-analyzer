...
- remaining_time: 画面上部中央のタイマーに表示されている残り時間を「M:SS」形式で記録。読み取れない場合はnull

JSONのみで回答してください:
{"kills": 1, "assists": 1, "special": 1, "is_dead": false, "remaining_time": null,
"my_team_color": "自チームの色", "enemy_team_color": "相手チームの色",
"my_team_count": null, "enemy_team_count": null, "description": "現在の状況の説明"}
スコアは全て1-10。不明なら1。is_deadはtrue/false。remaining_timeとゲームカウントが不明ならnull。"""


def build_frame_prompt(match_duration: str | None = None) -> str:
    """試合時間コンテキスト付きのフレーム分析プロンプトを生成する."""
    if match_duration is None:
        return FRAME_ANALYSIS_PROMPT
    return f"■ 試合時間: この試合は{match_duration}マッチです。\n\n{FRAME_ANALYSIS_PROMPT}"
